...
describe('action', () => {
  ...
  it('sets the token-info output with username', async () => {
    ...
    expect(downloadToolMock).toHaveBeenNthCalledWith(1, main.CRED_PROVIDER_URL)
    ...
    expect(setOutputMock).toHaveBeenNthCalledWith(1, 'cred-provider-dir', expectedDest)
    expect(setOutputMock).toHaveBeenNthCalledWith(2, 'token-info', JSON.stringify({
  ...
  it('allows custom credential provider URL', async () => {
    getInputMock.mockImplementation((name: string): string => {
      switch (name) {
        case 'username':
          return 'my-username'
    ...
    expect(runMock).toHaveReturned()
    expect(downloadToolMock).toHaveBeenNthCalledWith(1, 'https://my-custom-url')
    const expectedDest = path.join(os.homedir(), '.nuget', 'plugins', 'netcore', 'NuGet.TokenCredentialProvider')
    expect(extractZipMock).toHaveBeenNthCalledWith(1, 'tool-cache-path', expectedDest)
    expect(setOutputMock).toHaveBeenNthCalledWith(1, 'cred-provider-dir', expectedDest)
    expect(setOutputMock).toHaveBeenNthCalledWith(2, 'token-info', JSON.stringify({
  ...
  it('requires ACTIONS_RUNTIME_TOKEN env var', async () => {
    ...
    expect(runMock).toHaveReturned()
    expect(setFailedMock).toHaveBeenNthCalledWith(1, "The 'ACTIONS_RUNTIME_TOKEN' environment variable must be set. Does this workflow have 'id-token: write' permissions?")
    expect(setOutputMock).not.toHaveBeenCalled()
    expect(errorMock).not.toHaveBeenCalled()
  })
  ...
  it('requires ACTIONS_ID_TOKEN_REQUEST_URL env var', async () => {
    ...
    expect(runMock).toHaveReturned()
    expect(setFailedMock).toHaveBeenNthCalledWith(1, "The 'ACTIONS_ID_TOKEN_REQUEST_URL' environment variable must be set. Does this workflow have 'id-token: write' permissions?")
    expect(setOutputMock).not.toHaveBeenCalled()
    expect(errorMock).not.toHaveBeenCalled()
  })
})
